...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Lynx"),
        "preview" => ("app-icon-preview.ico", "Lynx Preview"),
        "nightly" => ("app-icon-nightly.ico", "Lynx Nightly"),
        _ => ("app-icon-dev.ico", "Lynx Dev"),
    };

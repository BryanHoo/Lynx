    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Lynx by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Lynx by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new("Base Keymap")).child(
        ToggleButtonGroup::two_rows(
            ...
            [
                ToggleButtonWithIcon::new("Lynx", IconName::AiZed, |_, _, cx| {
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Lynx can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all Lynx's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
        ...
        .map(|this| {
            if is_signed_in && is_free {
                ...
                this.on_click(move |_, _, cx| {
                    telemetry::event!("Welcome Lynx Agent Sign In Clicked");
                    let client = Client::global(cx);

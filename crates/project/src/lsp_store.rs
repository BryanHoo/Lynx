...
//!
//! [`LocalLspStore`] manages local language server lifecycles, while [`LspStore`]
//! provides the project-facing interface. Protocol handling is implemented by the
//! [`lsp`] crate.
pub mod clangd_ext;
pub mod code_lens;
mod document_colors;
...
use project_models::{ErrorCode, ErrorExt as _};
use rand::prelude::*;
use semver::Version;
use serde::Serialize;
...
impl ProgressToken {
    ...
    fn from_proto(value: project_models::ProgressToken) -> Option<Self> {
        Some(match value.value? {
            project_models::progress_token::Value::Number(number) => Self::Number(number),
            project_models::progress_token::Value::String(string) => {
                Self::String(SharedString::new(string))
            }
        })
    }

    fn to_proto(&self) -> project_models::ProgressToken {
        project_models::ProgressToken {
            value: Some(match self {
                Self::Number(number) => project_models::progress_token::Value::Number(*number),
                Self::String(string) => {
                    project_models::progress_token::Value::String(string.to_string())
                }
            }),
        }
    }
        ...
        for (server, adapter) in servers_and_adapters {
            ...
            if registered {
                cx.emit(LspStoreEvent::LanguageServerUpdate {
                    ...
                    name: None,
                    message: project_models::update_language_server::Variant::RegisteredForBuffer(
                        project_models::RegisteredForBuffer {
...
pub enum LspStoreEvent {
    ...
    LanguageServerUpdate {
        ...
        message: project_models::update_language_server::Variant,
    },
...
impl LspStore {
    pub fn as_local(&self) -> Option<&LocalLspStore> {
        let LspStoreMode::Local(local_lsp_store) = &self.mode;
        Some(local_lsp_store)
    }

    pub fn as_local_mut(&mut self) -> Option<&mut LocalLspStore> {
        let LspStoreMode::Local(local_lsp_store) = &mut self.mode;
        Some(local_lsp_store)
    }
    ...
    fn notify_server_capabilities_updated(
        ...
        // 动态注册直接更新 LanguageServer；同时刷新本地查询使用的能力缓存。
        self.lsp_server_capabilities
            .insert(server.server_id(), server.capabilities());
        let message = project_models::update_language_server::Variant::MetadataUpdated(
            project_models::ServerMetadataUpdated {
                capabilities: None,
                binary: Some(project_models::LanguageServerBinaryInfo {
                    ...
                    for node in nodes {
                        ...
                        if let Some(language_server_id) = server_id {
                            messages_to_report.push(LspStoreEvent::LanguageServerUpdate {
                                ...
                                name: node.name(),
                                message:
                                    project_models::update_language_server::Variant::RegisteredForBuffer(
                                        project_models::RegisteredForBuffer {
    ...
    fn apply_on_type_formatting(
        ...
        let Some(local) = self.as_local_mut() else {
            return Task::ready(Err(anyhow!("no local language server")));
        };
        let buffer_id = buffer.read(cx).remote_id();
    ...
    pub fn completions(
        ...
        let lsp_timeout = (completion_settings.lsp_fetch_timeout_ms > 0)
            .then(|| Duration::from_millis(completion_settings.lsp_fetch_timeout_ms));
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |lsp_store, cx| {
                for server_id in server_ids {
                    ...
                    let lsp_timeout =
                        lsp_timeout.map(|timeout| cx.background_executor().timer(timeout));
                    let mut timeout = cx
                            ...
                            timeout_happened = timeout => {
                                if timeout_happened {
                                    log::warn!(
                                        ...
                                        completion_settings.lsp_fetch_timeout_ms
                                    );
                                    Ok(None)
                                } else {
                                    Ok(Some(lsp_request.await?))
                                }
                            },
                        }
                    });
                    tasks.push((lsp_adapter, task));
                }
            })?;

            let futures = tasks.into_iter().map(async |(lsp_adapter, task)| {
                let response = task.await.ok()??;
    ...
    pub fn open_buffer_for_symbol(
        ...
        let Some(local) = self.as_local() else {
            return Task::ready(Err(anyhow!("no local language server store")));
        };
        ...
        let symbol_uri = match lsp::Uri::from_file_path(symbol_abs_path) {
            Ok(uri) => uri,
            Err(()) => return Task::ready(Err(anyhow!("invalid symbol path"))),
        };
    ...
    pub fn disk_based_diagnostics_started(
        ...
        cx.emit(LspStoreEvent::LanguageServerUpdate {
            ...
            message: project_models::update_language_server::Variant::DiskBasedDiagnosticsUpdating(
                Default::default(),
            ),
        })
    }
    ...
    pub fn disk_based_diagnostics_finished(
        ...
        cx.emit(LspStoreEvent::LanguageServerUpdate {
            ...
            name: self
                .language_server_adapter_for_id(language_server_id)
                .map(|adapter| adapter.name()),
            message: project_models::update_language_server::Variant::DiskBasedDiagnosticsUpdated(
                Default::default(),
            ),
        })
    }
    ...
    fn on_lsp_work_start(
        ...
        cx.emit(LspStoreEvent::LanguageServerUpdate {
            ...
            name: self
                .language_server_adapter_for_id(language_server_id)
                .map(|adapter| adapter.name()),
            message: project_models::update_language_server::Variant::WorkStart(
                project_models::LspWorkStart {
        ...
        if did_update {
            cx.emit(LspStoreEvent::LanguageServerUpdate {
                ...
                name: self
                    .language_server_adapter_for_id(language_server_id)
                    .map(|adapter| adapter.name()),
                message: project_models::update_language_server::Variant::WorkProgress(
                    project_models::LspWorkProgress {
                        token: Some(token.to_proto()),
                        message: progress.message,
                        percentage: progress.percentage.map(|p| p as u32),
                        is_cancellable: Some(progress.is_cancellable),
    ...
    fn on_lsp_work_end(
        ...
        cx.emit(LspStoreEvent::LanguageServerUpdate {
            ...
            name: self
                .language_server_adapter_for_id(language_server_id)
                .map(|adapter| adapter.name()),
            message: project_models::update_language_server::Variant::WorkEnd(
                project_models::LspWorkEnd {
                    token: Some(token.to_proto()),
                },
            ),
        })
    ...
    fn serialize_symbol(symbol: &Symbol) -> project_models::Symbol {
        let mut result = project_models::Symbol {
            language_server_name: symbol.language_server_name.0.to_string(),
            source_worktree_id: symbol.source_worktree_id.to_proto(),
            language_server_id: symbol.source_language_server_id.to_proto(),
            name: symbol.name.clone(),
            kind: symbol.kind.to_proto(),
            start: Some(project_models::PointUtf16 {
                row: symbol.range.start.0.row,
                column: symbol.range.start.0.column,
            }),
            end: Some(project_models::PointUtf16 {
    ...
    fn deserialize_symbol(serialized_symbol: project_models::Symbol) -> Result<CoreSymbol> {
    ...
    pub(crate) fn serialize_completion(completion: &CoreCompletion) -> project_models::Completion {
        let mut serialized_completion = project_models::Completion {
            ...
            ..project_models::Completion::default()
        };
        match &completion.source {
            CompletionSource::Lsp {
                insert_range,
                ...
                serialized_completion.source = project_models::completion::Source::Lsp as i32;
            ...
            CompletionSource::BufferWord {
                ...
                serialized_completion.source =
                    project_models::completion::Source::BufferWord as i32;
            ...
            CompletionSource::Custom => {
                serialized_completion.source = project_models::completion::Source::Custom as i32;
                serialized_completion.resolved = true;
            }
            CompletionSource::Dap { sort_text } => {
                serialized_completion.source = project_models::completion::Source::Dap as i32;
    ...
    pub(crate) fn deserialize_completion(
        completion: project_models::Completion,
    ) -> Result<CoreCompletion> {
        ...
        Ok(CoreCompletion {
            ...
            source: match project_models::completion::Source::try_from(completion.source).ok() {
                Some(project_models::completion::Source::Custom) => CompletionSource::Custom,
                Some(project_models::completion::Source::Lsp) => CompletionSource::Lsp {
                    insert_range,
                ...
                Some(project_models::completion::Source::BufferWord) => {
                ...
                Some(project_models::completion::Source::Dap) => CompletionSource::Dap {
    ...
    pub(crate) fn serialize_code_action(action: &CodeAction) -> project_models::CodeAction {
        let (kind, lsp_action) = match &action.lsp_action {
            LspAction::Action(code_action) => (
                project_models::code_action::Kind::Action as i32,
                serde_json::to_vec(code_action).unwrap(),
            ),
            LspAction::Command(command) => (
                project_models::code_action::Kind::Command as i32,
                serde_json::to_vec(command).unwrap(),
            ),
            LspAction::CodeLens(code_lens) => (
                project_models::code_action::Kind::CodeLens as i32,
                serde_json::to_vec(code_lens).unwrap(),
            ),
        };

        project_models::CodeAction {
    ...
    pub(crate) fn deserialize_code_action(
        action: project_models::CodeAction,
    ) -> Result<CodeAction> {
        let start = action
            .start
            .and_then(deserialize_anchor)
        ...
        let end = action
            ...
            .and_then(deserialize_anchor)
            .context("invalid end")?;
        let lsp_action = match project_models::code_action::Kind::try_from(action.kind).ok() {
            Some(project_models::code_action::Kind::Action) => {
                LspAction::Action(serde_json::from_slice(&action.lsp_action)?)
            }
            Some(project_models::code_action::Kind::Command) => {
                LspAction::Command(serde_json::from_slice(&action.lsp_action)?)
            }
            Some(project_models::code_action::Kind::CodeLens) => {
            ...
            if lsp_store
                .update(cx, |lsp_store, cx| {
                    ...
                    let binary_status = match binary_status {
                        BinaryStatus::None => project_models::ServerBinaryStatus::None,
                        ...
                        BinaryStatus::Starting => project_models::ServerBinaryStatus::Starting,
                        BinaryStatus::Stopping => project_models::ServerBinaryStatus::Stopping,
                        BinaryStatus::Stopped => project_models::ServerBinaryStatus::Stopped,
                    ...
                    cx.emit(LspStoreEvent::LanguageServerUpdate {
                        ...
                        message: project_models::update_language_server::Variant::StatusUpdate(
                            project_models::StatusUpdate {
                                message,
                                status: Some(project_models::status_update::Status::Binary(
...
impl LanguageServerLogType {
    pub fn to_proto(&self) -> project_models::language_server_log::LogType {
        match self {
            Self::Log(log_type) => {
                use project_models::log_message::LogLevel;
                ...
                project_models::language_server_log::LogType::Log(project_models::LogMessage {
                    level: level as i32,
                })
            }
            Self::Trace { verbose_info } => {
                project_models::language_server_log::LogType::Trace(project_models::TraceMessage {
                    verbose_info: verbose_info.to_owned(),
                })
            }
            Self::Rpc { received, elapsed } => {
                let kind = if *received {
                    project_models::rpc_message::Kind::Received
                } else {
                    project_models::rpc_message::Kind::Sent
                };
                let kind = kind as i32;
                let elapsed_nanos =
                    elapsed.map(|elapsed| u64::try_from(elapsed.as_nanos()).unwrap_or(u64::MAX));
                project_models::language_server_log::LogType::Rpc(project_models::RpcMessage {
                    kind,
                    elapsed_nanos,
                })
            }
    ...
    pub fn from_proto(log_type: project_models::language_server_log::LogType) -> Self {
        use project_models::log_message::LogLevel;
        use project_models::rpc_message;
        match log_type {
            project_models::language_server_log::LogType::Log(message_type) => Self::Log(
            ...
            project_models::language_server_log::LogType::Trace(trace_message) => Self::Trace {
                verbose_info: trace_message.verbose_info,
            },
            project_models::language_server_log::LogType::Rpc(message) => Self::Rpc {
...
impl DiagnosticSummary {
    ...
    pub fn to_proto(
        ...
        language_server_id: LanguageServerId,
        path: &RelPath,
    ) -> project_models::DiagnosticSummary {
        project_models::DiagnosticSummary {

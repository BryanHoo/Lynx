                            ...
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path().as_std_path()) {
                                    ...
                                    let zed_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into Lynx ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
                                            "serializing Lynx tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let zed_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into Lynx ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
...
pub fn local_settings_kind_from_proto(
    kind: project_models::LocalSettingsKind,
) -> LocalSettingsKind {
    match kind {
        project_models::LocalSettingsKind::Settings => LocalSettingsKind::Settings,
        project_models::LocalSettingsKind::Tasks => LocalSettingsKind::Tasks,
        project_models::LocalSettingsKind::Editorconfig => LocalSettingsKind::Editorconfig,
        project_models::LocalSettingsKind::Debug => LocalSettingsKind::Debug,
    }
}

pub fn local_settings_kind_to_proto(kind: LocalSettingsKind) -> project_models::LocalSettingsKind {
    match kind {
        LocalSettingsKind::Settings => project_models::LocalSettingsKind::Settings,
        LocalSettingsKind::Tasks => project_models::LocalSettingsKind::Tasks,
        LocalSettingsKind::Editorconfig => project_models::LocalSettingsKind::Editorconfig,
        LocalSettingsKind::Debug => project_models::LocalSettingsKind::Debug,
    }
}

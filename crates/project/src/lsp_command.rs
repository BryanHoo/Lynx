...
impl GetDocumentDiagnostics {
    pub fn deserialize_lsp_diagnostic(
        diagnostic: project_models::LspDiagnostic,
        ...
        let tags = diagnostic
            ...
            .into_iter()
            .filter_map(
                |tag| match project_models::LspDiagnosticTag::try_from(tag).ok() {
                    Some(project_models::LspDiagnosticTag::Unnecessary) => {
                        Some(lsp::DiagnosticTag::UNNECESSARY)
                    }
                    Some(project_models::LspDiagnosticTag::Deprecated) => {
                        Some(lsp::DiagnosticTag::DEPRECATED)
                    }
                    _ => None,
                },
            )
        ...
        Ok(lsp::Diagnostic {
            ...
            severity: match project_models::lsp_diagnostic::Severity::try_from(diagnostic.severity)
                .ok()
                .unwrap()
            {
                project_models::lsp_diagnostic::Severity::Error => {
                    Some(lsp::DiagnosticSeverity::ERROR)
                }
                project_models::lsp_diagnostic::Severity::Warning => {
                    Some(lsp::DiagnosticSeverity::WARNING)
                }
                project_models::lsp_diagnostic::Severity::Information => {
                    Some(lsp::DiagnosticSeverity::INFORMATION)
                }
                project_models::lsp_diagnostic::Severity::Hint => {
    ...
    pub fn serialize_lsp_diagnostic(
    ...
    ) -> Result<project_models::LspDiagnostic> {
        ...
        let related_information = diagnostic
            ...
            .map(|related_information| {
                ...
                Ok(project_models::LspDiagnosticRelatedInformation {
                    location_url: Some(related_information.location.uri.to_string()),
                    location_range_start: Some(project_models::PointUtf16 {
                        row: location_range_start.row,
                        column: location_range_start.column,
                    }),
                    location_range_end: Some(project_models::PointUtf16 {
        ...
        let tags = diagnostic
            ...
            .into_iter()
            .map(|tag| match tag {
                lsp::DiagnosticTag::UNNECESSARY => project_models::LspDiagnosticTag::Unnecessary,
                lsp::DiagnosticTag::DEPRECATED => project_models::LspDiagnosticTag::Deprecated,
                _ => project_models::LspDiagnosticTag::None,
            } as i32)
            .collect();

        Ok(project_models::LspDiagnostic {
            ...
            severity: match diagnostic.severity {
                ...
                _ => project_models::lsp_diagnostic::Severity::None,
            } as i32,

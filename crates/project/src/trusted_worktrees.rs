    ...
    match TrustedWorktrees::try_get_global(cx) {
        Some(trusted_worktrees) => {
            trusted_worktrees.update(cx, |trusted_worktrees, cx| {
                trusted_worktrees.add_worktree_store(worktree_store.clone(), remote_host, cx);
            });
        }
...
impl PathTrust {
    fn to_proto(&self) -> project_models::PathTrust {
        match self {
            Self::Worktree(worktree_id) => project_models::PathTrust {
                content: Some(project_models::path_trust::Content::WorktreeId(
                    worktree_id.to_proto(),
                )),
            },
            Self::AbsPath(path_buf) => project_models::PathTrust {
                content: Some(project_models::path_trust::Content::AbsPath(
    ...
    pub fn from_proto(proto: project_models::PathTrust) -> Option<Self> {
        Some(match proto.content? {
            project_models::path_trust::Content::WorktreeId(id) => {
                Self::Worktree(WorktreeId::from_proto(id))
            }
            project_models::path_trust::Content::AbsPath(path) => {
                Self::AbsPath(PathBuf::from(path))
            }
        })
    }
}

...
It contains our back-end logic for collaboration, to which we connect from the Lynx client via a websocket after authenticating via https://zed.dev, which is a separate repo running on Vercel.
...
In one terminal, run Lynx's collaboration server and the livekit dev server:

```sh
foreman start
```

In a second terminal, run two or more instances of Lynx.

```sh
script/zed-local -2
```

This script starts one to four instances of Lynx, depending on the `-2`, `-3` or `-4` flags. Each instance will be connected to the local `collab` server, signed in as a different user from `seed.json` or `seed.default.json`.

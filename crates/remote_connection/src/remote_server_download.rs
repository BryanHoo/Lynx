...
pub(crate) async fn download_remote_server_release(
    ...
    let release = get_release_asset(release_channel, version, os, arch, cx).await?;
...
pub(crate) async fn get_remote_server_release_url(
    release_channel: ReleaseChannel,
    version: Option<Version>,
    os: &str,
    arch: &str,
    ...
    Ok(Some(
        get_release_asset(release_channel, version, os, arch, cx)
            .await?
            .url,
    ))
}
...
async fn download_remote_server_binary(
    ...
    let temporary_file = tempfile::Builder::new().tempfile_in(paths::remote_servers_dir())?;
    let mut output = File::create(&temporary_file).await?;
    let mut response = http_client
    ...
    smol::io::copy(response.body_mut(), &mut output).await?;
    smol::fs::rename(&temporary_file, target_path).await?;
    Ok(())
}
...
async fn cleanup_remote_server_cache(
    ...
    for (index, (path, _)) in candidates.into_iter().enumerate() {
        if index >= limit
            && path != keep_path
            && let Err(error) = smol::fs::remove_file(&path).await
        {
            log::warn!(

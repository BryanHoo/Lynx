...
pub(super) fn translate(message: &str) -> Option<&'static str> {
    Some(match message {
        "(Linux only) choose how window control buttons are laid out in the titlebar." => {
            "（仅 Linux）选择窗口控制按钮在标题栏中的排列方式"
        }
        "(Linux only) whether Lynx or your compositor should draw window decorations." => {
            "（仅 Linux）由 Lynx 还是合成器绘制窗口装饰。"
        }
        "(Linux only) whether Zed or your compositor should draw window decorations." => {
            "（仅 Linux）指定由 Lynx 还是合成器绘制窗口装饰"
        }
        "(macOS only) whether to allow Windows to tab together." => {
            "（仅 macOS）是否允许将窗口合并为标签页"
        }
        "(macOS only) which fullscreen mode the toggle fullscreen action enters." => {
            "（仅 macOS）切换全屏操作所进入的全屏模式"
        }
        "(optional)" => "（可选）",
        "1 rule" => "1 条规则",
        "1 tool" => "1 个工具",
        ...
        "Add {}-Compatible Provider" => "添加兼容 {} 的提供商",
        "Additional code actions to run when formatting." => "格式化时运行的附加代码操作",
        ...
        "Agents connected through the Agent Client Protocol." => {
            "通过 Agent Client Protocol 连接的智能体"
        }
        "Allow" => "允许",
        ...
        "Allow sending requests to Anthropic models that cannot be offered with Zero Data Retention." => {
            "允许向无法以 Zero Data Retention 方式提供的 Anthropic 模型发送请求"
        }
        "Allowed" => "允许",
        "Allowed Domains" => "允许的域名",
        "Alternate Scroll" => "交替滚动",
        "Always Allow" => "始终允许",
        "Always Confirm" => "始终确认",
        "Always Deny" => "始终拒绝",
        "Always Treat Brackets As Autoclosed" => "始终将括号视为自动闭合",
        "Amount of indentation for nested items." => "嵌套项的缩进量",
        "Amount of time to wait before changing focus." => "更改焦点前的等待时间",
        "An agent named \"{}\" already exists." => "名为“{}”的智能体已存在",
        ...
        "Any number of settings profiles that are temporarily applied on top of your existing user settings." => {
            "可临时叠加应用于现有用户设置之上的任意数量的设置配置文件"
        }
        "Appearance" => "外观",
        "Arguments" => "参数",
        "Audible Bell" => "响铃",
        "Authenticate" => "验证身份",
        "Authenticate to connect this server" => "验证身份以连接此服务器",
        ...
        "Auto Open Files On Create" => "创建文件时自动打开",
        "Auto Open Files On Drop" => "拖入文件时自动打开",
        "Auto Open Files On Paste" => "粘贴文件时自动打开",
        ...
        "Automatically compact the agent's context when it grows too large, summarizing earlier messages to free up room in the model's context window." => {
            "在智能体的上下文增长过大时自动压缩，通过汇总较早的消息来为模型的上下文窗口腾出空间"
        }
        "Automatically show a signature help pop-up." => "自动显示签名帮助弹出框",
        "Autoscroll On Clicks" => "点击时自动滚动",
        "Background Coloring" => "背景着色",
        "Base Keymap" => "基础键映射",
        "Behavior Settings" => "行为设置",
        "Body is required." => "技能内容为必填项",
        ...
        "Collapses the navigation entry." => "折叠导航条目",
        "Collect timing data for foreground and background executor tasks so they can be inspected via `zed: open performance profiler`. May lead to increased memory usage." => {
            "收集前台和后台执行器任务的计时数据，以便通过 `zed: open performance profiler` 进行检查，可能导致内存使用增加"
        }
        "Coloring" => "着色",
        ...
        "Command to automatically run when Lynx creates a Terminal Thread shell in the agent panel. Runs in your configured shell." => {
            "Lynx 在智能体面板中创建终端对话 Shell 时自动运行的命令。该命令会在已配置的 Shell 中执行。"
        }
        "Command to automatically run when Zed creates a Terminal Thread shell in the agent panel. Runs in your configured shell." => {
            "Lynx 在智能体面板中创建终端会话 shell 时自动运行的命令。该命令会在你配置的 shell 中运行"
        }
        ...
        "Controls line number display in the editor's gutter. \"disabled\" shows absolute line numbers, \"enabled\" shows relative line numbers for each absolute line, and \"wrapped\" shows relative line numbers for every line, absolute or wrapped." => {
            "控制编辑器行号区的行号显示方式。\"disabled\" 显示绝对行号，\"enabled\" 对每个绝对行显示相对行号，\"wrapped\" 对所有行（包括折行）显示相对行号"
        }
        ...
        "Controls whether Zed may collect training data when using Zed's Edit Predictions. Data is only collected for files in projects detected as open source. The default value uses the preference previously set via the status-bar toggle, or false if no preference has been stored." => {
            "控制 Lynx 是否可在使用编辑预测时收集训练数据。仅对检测为开源项目中的文件收集数据。默认值使用之前通过状态栏开关设置的偏好，若未存储偏好则为 false。"
        }
        ...
        "Controls whether the closing characters are always skipped over and auto-removed no matter how they were inserted." => {
            "控制是否始终跳过并自动删除闭合字符，无论它们是如何插入的"
        }
        "Copy Link" => "复制链接",
        "Copy On Select" => "选中时复制",
        "Copy Path" => "复制路径",
        "Copy Share Link" => "复制共享链接",
        "Couldn't read shared skill: {err}" => "无法读取共享技能：{err}",

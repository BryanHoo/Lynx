...
pub fn prevent_root_execution() {
    ...
    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running Lynx as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of Lynx.
...
pub fn get_shell_safe_zed_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    ...
    zed_path
        ...
        .context("Failed to shell-escape Lynx executable path.")
}

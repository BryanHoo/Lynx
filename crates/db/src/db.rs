...
mod tests {
    ...
    async fn test_bad_migration_panics() {
        ...
        let tempdir = tempfile::Builder::new()
            ...
            .tempdir()
            .unwrap();
        let _bad_db = open_db::<BadDB>(tempdir.path(), release_channel::ReleaseChannel::Lynx).await;
    }
    ...
    async fn test_db_corruption(cx: &mut gpui::TestAppContext) {
        ...
        {
            let corrupt_db =
                open_db::<CorruptedDB>(tempdir.path(), release_channel::ReleaseChannel::Lynx).await;
            assert!(corrupt_db.persistent());
        }

        let good_db =
            open_db::<GoodDB>(tempdir.path(), release_channel::ReleaseChannel::Lynx).await;
        assert!(

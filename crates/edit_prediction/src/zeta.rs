...
use gpui::{AppContext as _, Task, prelude::*};
use language::{
    BufferSnapshot, DiagnosticSeverity, EditPredictionPromptFormat, OffsetRangeExt as _,
...
use zeta_prompt::{
    ParsedOutput, ZetaFormat, excerpt_ranges_for_format, format_zeta_prompt, get_prefill,
...
pub(crate) fn request_prediction_with_zeta(
    ...
    let Some(custom_server_settings) = custom_server_settings else {
    ...
    let request_task = cx.background_spawn(async move {
        let local_zeta_version = match custom_server_settings.prompt_format {
            EditPredictionPromptFormat::Zeta(version) => Some(version),
            EditPredictionPromptFormat::Infer => {
                match custom_server_settings.model.to_ascii_lowercase().as_str() {
        ...
        let max_tokens = custom_server_settings.max_output_tokens * 4;
        let (request_id, output) = match local_zeta_version {
            ZetaVersion::Zeta1 => {
        ...
        Ok::<_, anyhow::Error>(Some((
            request_id,
            EditPredictionInputs::V2(prompt_input),
            buffer,
            snapshot,
            edits,
            cursor_position,
    ...
    cx.spawn(async move |_this, cx| {
        ...
        let editable_range = edited_buffer_snapshot.anchor_range_inside(editable_range_in_buffer);
        Ok(Some(

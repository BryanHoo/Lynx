...
impl WindowTitleContext {
    fn value_for(&self, variable: &str) -> Option<&str> {
        match variable {
            ...
            // native templates navigate_with_agent the same rendering rules.
            _ => None,
        }
    }
}
...
#[derive(Default)]
pub struct AgentNavigableItemRegistry(TypeIdHashMap<AgentNavigableItemDescriptor>);

struct AgentNavigableItemDescriptor {
    to_agent_navigable_view: fn(&AnyView) -> Box<dyn AgentNavigableItemHandle>,
}

impl Global for AgentNavigableItemRegistry {}

impl AgentNavigableItemRegistry {
    pub fn register<I: AgentNavigableItem>(cx: &mut App) {
        cx.default_global::<Self>().0.insert(
            TypeId::of::<I>(),
            AgentNavigableItemDescriptor {
                to_agent_navigable_view: |view| Box::new(view.clone().downcast::<I>().unwrap()),
            },
        );
    }

    pub fn to_agent_navigable_view(
        view: impl Into<AnyView>,
        cx: &App,
    ) -> Option<Box<dyn AgentNavigableItemHandle>> {
        let this = cx.try_global::<Self>()?;
        let view = view.into();
        let descriptor = this.0.get(&view.entity_type())?;
        Some((descriptor.to_agent_navigable_view)(&view))
    }
}
...
pub struct Workspace {
    ...
    project: Entity<Project>,
    agent_navigation_states: HashMap<AgentNavigationTarget, AgentNavigationState>,
    last_leaders_by_pane: HashMap<WeakEntity<Pane>, AgentNavigationTarget>,
    window_edited: bool,
...
pub struct AgentNavigationViewId {
    pub id: u64,
}
...
pub struct AgentNavigationState {
    ...
    active_agent_view_id: Option<AgentNavigationViewId>,
    items_by_agent_view_id: HashMap<AgentNavigationViewId, AgentNavigationView>,
}

struct AgentNavigationView {
    view: Box<dyn AgentNavigableItemHandle>,
}
...
impl Workspace {
    pub fn new(
        ...
        cx.subscribe_in(&project, window, move |this, _, event, window, cx| {
            match event {
                ...
                project::Event::DisconnectedFromHost => {
                    ...
                    let navigation_targets_to_stop = this
                        .agent_navigation_states
                        .keys()
                        .copied()
                        .collect::<Vec<_>>();
                    for agent_navigation_target in navigation_targets_to_stop {
                        this.stop_agent_navigation(agent_navigation_target, window, cx);
                    }
                }
        ...
        Workspace {
            ...
            agent_navigation_states: Default::default(),
    ...
    fn handle_pane_event(
        ...
        match event {
            ...
            pane::Event::ActivateItem {
                ...
                if *local {
                    self.stop_agent_navigation_in_pane(pane, window, cx);
                }
    ...
    pub fn stop_agent_navigation_in_pane(
        &mut self,
        pane: &Entity<Pane>,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) -> Option<AgentNavigationTarget> {
        let agent_navigation_target = self.agent_navigation_target_for_pane(pane)?;
        self.stop_agent_navigation(agent_navigation_target, window, cx);
        Some(agent_navigation_target)
    }
    ...
    pub fn start_agent_navigation(
        &mut self,
        agent_navigation_target: impl Into<AgentNavigationTarget>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Option<Task<Result<()>>> {
        let agent_navigation_target = agent_navigation_target.into();
        let pane = self.active_pane().clone();

        self.last_leaders_by_pane
            .insert(pane.downgrade(), agent_navigation_target);
        self.stop_agent_navigation(agent_navigation_target, window, cx);
        self.stop_agent_navigation_in_pane(&pane, window, cx);
        self.agent_navigation_states.insert(
            agent_navigation_target,
            AgentNavigationState {
                center_pane: pane.clone(),
                dock_pane: None,
                active_agent_view_id: None,
                items_by_agent_view_id: Default::default(),
            },
        );
        cx.notify();

        self.agent_navigation_updated(agent_navigation_target, window, cx)?;
        Some(Task::ready(Ok(())))
    }

    pub fn navigate_with_agent(
        &mut self,
        agent_navigation_target: impl Into<AgentNavigationTarget>,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        if let Some(navigation_state) = self.agent_navigation_states.get(&agent_navigation_target) {
            window.focus(&navigation_state.pane().focus_handle(cx), cx);

            return;
        }

        // Otherwise, navigate_with_agent.
        if let Some(task) = self.start_agent_navigation(agent_navigation_target, window, cx) {
            task.detach_and_log_err(cx)
        }
    }

    pub fn stop_agent_navigation(
        &mut self,
        agent_navigation_target: impl Into<AgentNavigationTarget>,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let state = self
            .agent_navigation_states
            .remove(&agent_navigation_target)?;
        for (_, item) in state.items_by_agent_view_id {
            item.view.set_agent_navigation_target(None, window, cx);
        }

        Some(())
    }

    pub fn is_navigating_with_agent(&self, id: impl Into<AgentNavigationTarget>) -> bool {
        self.agent_navigation_states.contains_key(&id.into())
    }
    ...
    fn handle_agent_location_changed(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(navigation_state) = self
            .agent_navigation_states
            .get_mut(&AgentNavigationTarget::Agent)
        else {
            return;
        };
        ...
        if let Some(agent_location) = self.project.read(cx).agent_location() {
            ...
            let view_id = AgentNavigationViewId {
                id: buffer_entity_id.as_u64(),
            };
            navigation_state.active_agent_view_id = Some(view_id);

            let item = match navigation_state.items_by_agent_view_id.entry(view_id) {
                ...
                hash_map::Entry::Vacant(entry) => {
                    let existing_view =
                        navigation_state
                            ...
                            .items()
                            .find_map(|item| {
                                let item = item.to_agent_navigable_item_handle(cx)?;
                                if item.buffer_kind(cx) == ItemBufferKind::Singleton
                    ...
                    let view = existing_view.or_else(|| {
                        agent_location.buffer.upgrade().and_then(|buffer| {
                            ...
                            .to_agent_navigable_item_handle(cx)
                        })
                    });

                    view.map(|view| entry.insert(AgentNavigationView { view }))
                }
            };

            if let Some(item) = item {
                item.view.set_agent_navigation_target(
                    Some(AgentNavigationTarget::Agent),
                    window,
                    cx,
                );
                item.view
            ...
            navigation_state.active_agent_view_id = None;
        }

        self.agent_navigation_updated(AgentNavigationTarget::Agent, window, cx);
    }

    pub fn agent_navigation_target_for_pane(
        &self,
        pane: &Entity<Pane>,
    ) -> Option<AgentNavigationTarget> {
        self.agent_navigation_states
            .iter()
            .find_map(|(agent_navigation_target, state)| {
    ...
    fn agent_navigation_updated(
        ...
        let state = self.agent_navigation_states.get(&agent_navigation_target)?;
        let mut transfer_focus = state.center_pane.read(cx).has_focus(window, cx);
        let pane;
        pane = state.center_pane.clone();
        let state = self
            .agent_navigation_states
            .get_mut(&agent_navigation_target)?;
    ...
    fn active_item_for_agent(&self) -> Option<Box<dyn ItemHandle>> {
        let state = self
            .agent_navigation_states
            .get(&AgentNavigationTarget::Agent)?;
        let active_agent_view_id = state.active_agent_view_id?;
        Some(
            state
                .items_by_agent_view_id
                .get(&active_agent_view_id)?
                .view
                .boxed_clone(),
        )
    }
    ...
    fn render_dock(
        ...
        let leader_border = dock.read(cx).active_panel().and_then(|panel| {
            ...
            let agent_navigation_states = &self.agent_navigation_states;
            leader_border_for_pane(agent_navigation_states, &pane, window, cx)
        });
...
impl Render for Workspace {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let pane_render_context = PaneRenderContext {
            agent_navigation_states: &self.agent_navigation_states,
...
pub fn workspace_windows_for_location(
    _serialized_location: &SerializedWorkspaceLocation,
    cx: &App,
            ...
            multi_workspace.read(cx).is_ok_and(|multi_workspace| {
                multi_workspace.workspaces().any(|workspace| {
                    match workspace.read(cx).workspace_location(cx) {
                        WorkspaceLocation::Location(SerializedWorkspaceLocation::Local, _) => true,
                        _ => false,
                    }
                })
            })
...
pub fn create_and_open_local_file(
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        if !fs.is_file(&requested_path).await {
            fs.create_file(&requested_path, Default::default()).await?;
            fs.save(&requested_path, &default_content(), Default::default())
                .await?;
        }
...
mod tests {
    ...
    fn test_window_title_needs_from_template() {
        ...
        let noise = WindowTitleNeeds::from_template("filePath relativePath ${projectName}");
    ...
    async fn test_window_title_format_setting(cx: &mut TestAppContext) {
        ...
        cx.update(|_, cx| {
            SettingsStore::update_global(cx, |settings, cx| {
                settings.update_user_settings(cx, |settings| {
                    settings.workspace.window_title_format =
                        Some("${projectName}${separator}${missing}".to_string());
                })
            });
        });
        cx.executor().run_until_parked();
        ...
        cx.update(|_, cx| {
            SettingsStore::update_global(cx, |settings, cx| {
                settings.update_user_settings(cx, |settings| {
                    settings.workspace.window_title_format = Some(" ${missing}".to_string());
                })
            });
        });
        cx.executor().run_until_parked();
    ...
    fn render_center_group(
        ...
        workspace.center.root.render(
            ...
            None,
            &PaneRenderContext {
                agent_navigation_states: &workspace.agent_navigation_states,
    ...
    async fn test_restoring_active_and_preview_tabs_when_items_fail_to_deserialize(
        ...
        pane.read_with(cx, |pane, _| {
            ...
            assert_eq!(
                ...
                "the preview tab should navigate_with_agent the item it was serialized with"
            );
        });

use crate::{
    AgentNavigableItemRegistry, AgentNavigationTarget, DelayedDebouncedEditAction, ItemNavHistory,
...
pub trait ItemHandle: 'static + Send {
    ...
    fn to_agent_navigable_item_handle(&self, cx: &App)
...
impl<T: Item> ItemHandle for Entity<T> {
    ...
    fn to_agent_navigable_item_handle(
        &self,
        cx: &App,
    ) -> Option<Box<dyn AgentNavigableItemHandle>> {
        AgentNavigableItemRegistry::to_agent_navigable_view(self.clone(), cx)
    }
...
pub trait AgentNavigableItem: Item {
    fn set_agent_navigation_target(
        &mut self,
        leader_peer_id: Option<AgentNavigationTarget>,
        window: &mut Window,
        cx: &mut Context<Self>,
    );
...
pub trait AgentNavigableItemHandle: ItemHandle {
    fn set_agent_navigation_target(
        &self,
        leader_peer_id: Option<AgentNavigationTarget>,
        window: &mut Window,
...
impl<T: AgentNavigableItem> AgentNavigableItemHandle for Entity<T> {
    fn set_agent_navigation_target(
        &self,
        ...
        self.update(cx, |this, cx| {
            this.set_agent_navigation_target(agent_navigation_target, window, cx)
        })
    }

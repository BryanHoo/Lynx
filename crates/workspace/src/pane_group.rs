use crate::{
    AgentNavigationState, AgentNavigationTarget, Pane, Workspace, WorkspaceSettings,
...
pub struct PaneRenderContext<'a> {
    pub agent_navigation_states: &'a HashMap<AgentNavigationTarget, AgentNavigationState>,
...
impl PaneLeaderDecorator for PaneRenderContext<'_> {
    fn decorate(&self, pane: &Entity<Pane>, cx: &App) -> LeaderDecoration {
        ...
        let is_in_panel = navigation_state.dock_pane.is_some();
        if is_in_panel {
            navigation_color.fade_out(0.75);
        } else {

...
#[command(
    ...
    before_help = "The Lynx CLI binary.
This CLI is a separate binary that invokes Lynx.
...
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/Lynx`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\Lynx`.")]
    #[cfg_attr(
    ...
    diff: Vec<String>,
    /// Generate shell completions for Lynx
    #[arg(long, value_names = ["SHELL"])]
    completions: Option<Shell>,
    /// Uninstall Lynx from user system
    #[cfg(all(
...
fn run() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Lynx CLI, only on Lynx binary.",
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Lynx spawn")?;
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after Lynx spawn")?;
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let prompt = format!(
        ...
        console::style("You can change this later in Lynx settings"),
    );
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("Lynx {} – {}", self.version(), self.path().display(),)
        }

...
async fn verify_created_by_zed(root: &RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    let created_at = receiver
        ...
        .with_context(|| {
            format!(
                "refusing to delete worktree at {}: failed to verify that Lynx created it",
    ...
    match created_at {
        ...
        Some(_) => {
            ...
            .log_err();
            Err(anyhow!(
                "refusing to delete worktree at {}: it is not the worktree Lynx created \
                 (it was likely removed and recreated outside Lynx)",
                root.root_path.display()
            ))
        }
    }
...
mod tests {
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the Lynx-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_remove_root_refuses_when_worktree_recreated_outside_zed(cx: &mut TestAppContext) {
        ...
        assert!(
            error.to_string().contains("not the worktree Lynx created"),

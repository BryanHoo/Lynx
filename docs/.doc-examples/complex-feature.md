...
> **Note:** You need to have an LLM provider configured either via your own API keys or through Lynx's hosted AI models.
...
These instructions are sent to the model in addition to any instruction files, such as `.rules` or `AGENTS.md`. To add instructions that apply to both commit messages and the agent more broadly, use the global `AGENTS.md` file located `~/.config/zed/AGENTS.md` on macOS and Linux, `%APPDATA%\Lynx\AGENTS.md` on Windows.
...
Lynx also has a Copy Permalink feature to create a permanent link to a code snippet on your Git hosting service.

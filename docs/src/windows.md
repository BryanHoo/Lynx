...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for Lynx, and click Uninstall.
...
- Check that your hardware and operating system version are compatible with Lynx. See our [installation guide](./installation.md) for more information.
...
- Try launching Lynx with no extensions or custom settings to isolate conflicts.
...
#### Lynx fails to open / degraded performance

Lynx requires a DirectX 11 compatible GPU to run. If Lynx fails to open, your GPU may not meet the minimum requirements.
...
If you're running Lynx inside a virtual machine, it will use the emulated adapter provided by your VM. While Lynx will work in this environment, performance may be degraded.

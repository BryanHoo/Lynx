---
title: Lynx on Linux
description: "The installation script on the download page is the fastest way to install Lynx:"
---
...
The installation script on the [download](https://zed.dev/download) page is the fastest way to install Lynx:

```sh
curl -f https://zed.dev/install.sh | sh
```

We also offer a preview build of Lynx which receives updates about a week ahead of stable. You can install it with:

```sh
curl -f https://zed.dev/install.sh | ZED_CHANNEL=preview sh
```

The Lynx installed by the script works best on systems that:
...
## Other ways to install Lynx on Linux

Lynx is open source, and [you can install from source](./development/linux.md).

### Installing via a package manager

There are several third-party Lynx packages for various Linux distributions and package managers, sometimes under `zed-editor`. Availability varies by distribution, but you may be able to install Lynx using one of these packages:
...
See [Repology](https://repology.org/project/zed-editor/versions) for a current list of Lynx packages in various repositories.

### Community

When installing a third-party package please be aware that it may not be completely up to date and may be slightly different from the Lynx we package (a common change is to rename the binary to `zedit` or `zeditor` to avoid conflicting with other packages).

We'd love your help making Lynx available for everyone. If Lynx is not yet available for your package manager, and you would like to fix that, we have some notes on [how to do it](./development/linux.md#notes-for-packaging-zed).

The packages in this section provide binary installs for Lynx but are not official packages within the associated distributions. These packages are maintained by community members and as such a higher level of caution should be taken when installing them.

### Downloading manually

If you'd prefer, you can install Lynx by downloading our pre-built .tar.gz. This is the same artifact that our install script uses, but you can customize the location of your installation by modifying the instructions below:
...
If Lynx was installed using the default installation script, it can be uninstalled by supplying the `--uninstall` flag to the `zed` shell command:

```sh
zed --uninstall
```

Note that this will uninstall the Lynx variant that the symlink points to. If you have multiple, parallel installations (such as both Stable and Preview), then you should instead use its absolute path as described further down in this section.

If there are no errors, the shell will then prompt you whether you'd like to keep your preferences or delete them. After making a choice, you should see a message that Lynx was successfully uninstalled.
...
The first case might fail if a symlink was not properly established between `$HOME/.local/bin/zed` and `$HOME/.local/zed.app/bin/zed`, or if the symlink was overwritten by having parallel installations of different versions of Lynx. But the second command should always work as long as Lynx was installed to its default location.

If Lynx was installed to a different location (such as the `zed-preview.app` variant), you must invoke the `zed` binary stored in that installation directory and pass the `--uninstall` flag to it in the same format as the previous commands.

### Package Manager

If Lynx was installed using a package manager, please consult the documentation for that package manager on how to uninstall a package.

## Troubleshooting

Linux works on a large variety of systems configured in many different ways. We primarily test Lynx on a vanilla Ubuntu setup, as it is the most common distribution our users use. That said, we do expect it to work on a wide variety of machines.

### Lynx fails to start

If you see an error like "/lib64/libc.so.6: version 'GLIBC_2.29' not found" it means that your distribution's version of glibc is too old. You can either upgrade your system, or [install Lynx from source](./development/linux.md).

### Graphics issues

#### Lynx fails to open windows

Lynx requires a GPU to run effectively. Under the hood, we use [Vulkan](https://www.vulkan.org/) to communicate with your GPU. If you are seeing problems with performance, or Lynx fails to load, it is possible that Vulkan is the culprit.

If you see a notification saying `Lynx failed to open a window: NoSupportedDeviceFound` this means that Vulkan cannot find a compatible GPU. You can try running [vkcube](https://github.com/krh/vkcube) (usually available as part of the `vulkaninfo` or `vulkan-tools` package on various distributions) to try to troubleshoot where the issue is coming from like so:
...
You can find out which graphics card Lynx is using by looking in the Lynx log (`~/.local/share/zed/logs/Zed.log`) for `Using GPU: ...`.
...
On others, you may be able to set the environment variable `DRI_PRIME=1` when running Lynx to force the use of the discrete GPU.
...
#### Forcing Lynx to use a specific GPU

There are a few different ways to force Lynx to use a specific GPU:

##### Option A

You can use the `ZED_DEVICE_ID={device_id}` environment variable to specify the device ID of the GPU you wish to have Lynx use.
...
where the device ID here is `2484`. This value is in hexadecimal, so to force Lynx to use this specific GPU you would set the environment variable like so:
...
If Vulkan is configured correctly, and Lynx is still not working for you, please [file an issue](https://github.com/zed-industries/zed) with as much information as possible.

When reporting issues where Lynx fails to start due to graphics initialization errors on GitHub, it can be impossible to run the {#action zed::CopySystemSpecsIntoClipboard} command like we instruct you to in our issue template. We provide an alternative way to collect the system specs specifically for this situation.

Passing the `--system-specs` flag to Lynx like
...
Additionally, it is extremely beneficial to provide the contents of your Lynx log when reporting such issues. The log is usually located at `~/.local/share/zed/logs/Zed.log`. The recommended process for producing a helpful log file is as follows:
...
Or, if you have the Lynx cli setup, you can do
...
````
<details><summary>Lynx Log</summary>
...
### Lynx isn't remembering my API keys

### Lynx isn't remembering my login
...
Lynx needs a place to securely store secrets such as your Lynx login cookie or your OpenAI API Keys and we use a system provided keychain to do this. Examples of packages that provide this are `gnome-keyring`, `KWallet` and `keepassxc` among others.

### Could not start inotify

Lynx relies on inotify to watch your filesystem for changes. If you cannot start inotify then Lynx will not work reliably.

If you are seeing "too many open files" then first try `sysctl fs.inotify`.

- You should see that max_user_instances is 128 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_instances=1024`). Lynx needs only 1 inotify instance.
- You should see that `max_user_watches` is 8000 or higher (you can change the limit with `sudo sysctl fs.inotify.max_user_watches=64000`). Lynx needs one watch per directory in all your open projects + one per git repository + a handful more for settings, themes, keymaps, extensions.

It is also possible that you are running out of file descriptors. You can check the limits with `ulimit` and update them by editing `/etc/security/limits.conf`.

### No sound or wrong output device

If you're not hearing any sound in Lynx or the audio is routed to the wrong device, it could be due to a mismatch between audio systems. Lynx relies on ALSA, while your system may be using PipeWire or PulseAudio. To resolve this, you need to configure ALSA to route audio through PipeWire/PulseAudio.
...
On X11 systems, Lynx automatically detects the appropriate scale factor for high-DPI displays. The scale factor is determined using the following priority order:
...
If you want to customize the scale factor beyond what Lynx detects automatically, you have several options:
...
If this command returns no output, Lynx is using RandR (X11's monitor management extension) to automatically calculate the scale factor based on your monitor's reported resolution and physical dimensions.

#### Option 1: Set Xft.dpi (X Resources Database)

`Xft.dpi` is a standard X11 setting that many applications use for consistent font and UI scaling. Setting this ensures Lynx scales the same way as other X11 applications that respect this setting.
...
Restart Lynx for the changes to take effect.

#### Option 2: Use the GPUI_X11_SCALE_FACTOR environment variable

This Lynx-specific environment variable directly sets the scale factor, bypassing all automatic detection.
...
Replace `192` with your desired DPI value. This affects the system globally and will be used by Lynx's automatic RandR detection when `Xft.dpi` is not set.

### Font rendering parameters

On Linux, Lynx reads `ZED_FONTS_GAMMA` and `ZED_FONTS_GRAYSCALE_ENHANCED_CONTRAST` environment variables for the values to use for font rendering.

---
title: Building Lynx for Windows
description: "Guide to building zed for windows for Lynx development."
---

# Building Lynx for Windows
...
If you cannot compile Lynx, make sure a Visual Studio installation includes at least the following components:
...
> **Note:** Visual regression tests are currently macOS-only and require Screen Recording permission. See [Building Lynx for macOS](./macos.md#visual-regression-tests) for details.

## Installing from msys2

Lynx does not support unofficial MSYS2 Lynx packages built for Mingw-w64. Please report any issues you may have with [mingw-w64-zed](https://packages.msys2.org/base/mingw-w64-zed) to [msys2/MINGW-packages/issues](https://github.com/msys2/MINGW-packages/issues?q=is%3Aissue+is%3Aopen+zed).
...
If you set the `RUSTFLAGS` env var, it will override the `rustflags` settings in `.cargo/config.toml` which is required to properly build Lynx.
...
Or, create a new `.cargo/config.toml` in the parent directory of the Lynx repo (see below). This is useful in CI because you do not need to edit the repo's original `.cargo/config.toml`.
...
If you are using a global config, consider moving the Lynx repository to a nested directory and add a `.cargo/config.toml` with a custom linker config in the parent directory.

See this issue for more information [#12041](https://github.com/zed-industries/zed/issues/12041)

### Invalid RC path selected

Sometimes, depending on the security rules applied to your laptop, you may get the following error while compiling Lynx:
...
#### Lynx fails to launch

Lynx currently uses Vulkan as its graphics API on Windows. If Lynx fails to launch, Vulkan is a common cause.

You can check the Lynx log at:
`C:\Users\YOU\AppData\Local\Zed\logs\Zed.log`

If you see messages like:

- `Lynx failed to open a window: NoSupportedDeviceFound`
...
If there's nothing Vulkan-related in the logs and you happen to have Bandicam installed, try uninstalling it. Lynx is currently not compatible with Bandicam.

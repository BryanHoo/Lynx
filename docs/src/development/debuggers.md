...
description: "Guide to using a debugger for Lynx development."
---

# Using a debugger

> This page is not about [configuring Lynx's debugger](../debugger.md).
> It covers how to use a debugger while developing Lynx itself.

## Using Lynx's built-in debugger

While the Lynx project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug Lynx with, one for GDB and one for LLDB. Select the configuration you want and Lynx will build and launch the binary.
...
To get the full debugger experience on a release build, compile a Lynx binary with full debug info.
...
## Running Lynx with a shell debugger GDB/LLDB

### Background

When you install Rust through rustup (the recommended setup for Lynx development; see your platform guide [here](../development.md)), rustup also installs helper scripts for debugging Rust binaries.
...
After enabling full debug info and building with `cargo build`, run `rust-gdb` or `rust-lldb` against the compiled Lynx binary:
...
You can also attach to a running Lynx process (for example, one started with `cargo run`):
...
`<pid>` is the process ID of the Lynx instance you want to attach to.
...
Debuggers are useful for finding the cause of panics and crashes, including in Lynx.
...
You can find additional information on debugging Lynx crashes [here](./debugging-crashes.md).

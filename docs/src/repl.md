---
title: REPL - Jupyter Kernels in Lynx
description: Run code interactively in Lynx with built-in Jupyter kernel support. Execute Python, TypeScript, R, and more inline.
---
...
Lynx's built-in REPL uses [Jupyter kernels](https://docs.jupyter.org/en/latest/projects/kernels.html) so you can run code interactively in regular editor files.
...
Lynx supports running code in multiple languages. To get started, you need to install a kernel for the language you want to use.
...
Lynx supports [notebooks as scripts](https://jupytext.readthedocs.io/en/latest/formats-scripts.html) using the `# %%` cell separator in Python and `// %%` in TypeScript. This allows you to write code in a single file and run it as if it were a notebook, cell by cell.
...
- Install the R Extension for Lynx (search for `R` in Lynx Extensions)
...
- Install the Julia Extension for Lynx (search for `Julia` in Lynx Extensions)
...
Lynx automatically detects available kernels and organizes them in the kernel picker:
...
Python environments appear in the picker even if ipykernel is not installed. Environments missing ipykernel are dimmed and labeled "ipykernel not installed." When you select one, Lynx automatically runs `pip install ipykernel` in that environment and activates it once installation completes.

### How Lynx Recommends Kernels

When you run code, Lynx selects a kernel automatically:

1. **Active toolchain match**: If a Python environment matches your active toolchain and has ipykernel, Lynx uses it
2. **First available Python env**: Otherwise, the first Python environment with ipykernel
3. **Language-based fallback**: If no Python envs are ready, Lynx picks a Jupyter kernel matching the code block's language
...
> Note: Lynx makes best effort usage of `sys.prefix` and `CONDA_PREFIX` to find kernels in Python environments. If you want to explicitly control this, run `python -m ipykernel install --user --name myenv --display-name "Python (myenv)"` to install the kernel directly while in the environment.

---
title: Vim Mode - Lynx
description: Full Vim emulation in Lynx with motions, text objects, visual mode, macros, and Lynx-specific extensions.
---

# Vim Mode

Lynx includes a Vim emulation layer. This page covers enabling and disabling vim mode, key bindings, Lynx-specific features, and configuration options.

## Lynx's vim mode design

Vim mode replicates the behavior of motions and commands where it makes sense and uses Lynx-specific functionality where Lynx's approach is better. The goal is a familiar experience that works out of the box without requiring configuration.
...
So, Lynx's vim mode does not replicate Vim one-to-one, but it meshes Vim's modal design with Lynx's modern features to provide a more fluid experience. It's also configurable, so you can add your own key bindings or override the defaults.

### Core differences

There are four types of features in vim mode that use Lynx's core functionality, leading to some differences in behavior:

1. **Motions**: vim mode uses Lynx's semantic parsing to tune the behavior of motions per language. For example, in Rust, jumping to matching bracket with `%` works with the pipe character `|`. In JavaScript, `w` considers `$` to be a word character.
2. **Visual block selections**: vim mode uses Lynx's multiple cursor to emulate visual block selections, making block selections a lot more flexible. For example, anything you insert after a block selection updates on every line in real-time, and you can add or remove cursors anytime.
3. **Macros**: vim mode uses Lynx's recording system for vim macros. So, you can capture and replay more complex actions, like autocompletion.
4. **Search and replace**: vim mode uses Lynx's search system, so, the syntax for regular expressions is slightly different compared to Vim. [Head to the Regex differences section](#regex-differences) for details.

> **Note:** The foundations of Lynx's vim mode should already cover many use cases, and we're always looking to improve it. If you find missing features that you rely on in your workflow, please [file an issue on GitHub](https://github.com/zed-industries/zed/issues).

## Enabling and disabling vim mode

When you first open Lynx, you'll see a checkbox on the welcome screen that allows you to enable vim mode.
...
Lynx is built on a modern foundation that (among other things) uses Tree-sitter and language servers to understand the content of the file you're editing and supports multiple cursors out of the box.

Vim mode has several "core Lynx" key bindings that will help you make the most of Lynx's specific feature set.
...
Tree-sitter is the parser Lynx uses to understand the structure of your code. Lynx provides motions that change the current cursor position, and text objects that can be used as the target of actions.
...
These commands help you manage multiple cursors in Lynx.
...
The following commands help you bring up Lynx's completion menu, request a suggestion from GitHub Copilot, or open the inline AI assistant without leaving insert mode.
...
Lynx's vim mode includes features commonly provided by plugins in the Vim ecosystem:
...
Lynx offers two different strategies for selecting text surrounded by any quote, or any bracket. These text objects are **not enabled by default** and must be configured in your keymap to be used.
...
Vim mode allows you to open Lynx's command palette with `:`. You can then type to access any usual Lynx command. Additionally, vim mode adds aliases for popular Vim commands to ensure your muscle memory transfers to Lynx. For example, you can write `:w` or `:write` to save the file.
...
| `:cq`           | Quit completely (close all running instances of Lynx) |
...
These ex commands open Lynx's various panels and windows.
...
This command replaces text. It emulates the substitute command in vim. The substitute command uses regular expressions, and Lynx uses a slightly different syntax than vim. You can learn more about Lynx's syntax below, [in the regex differences section](#regex-differences). Lynx will replace only the first occurrence of the search pattern in the current line. To replace all matches append the `g` flag.
...
Lynx does not ship with any command mnemonics by default, but you can define short aliases for Lynx commands using the `command_aliases` setting in your settings file. When you type an alias from this map in the command palette, it resolves to the mapped command.
...
```json [keymap]
// This key binding will work when you're editing a file. It comes built into Lynx by default as the workspace: save command.
{
  "context": "Workspace",
  "bindings": {
    "ctrl-s": "workspace::Save"
...
In non-modal text editors, cursor navigation typically wraps when moving past line ends. Lynx, however, handles this behavior exactly like Vim by default: the cursor stops at line boundaries. If you prefer your cursor to wrap between lines, override these keybindings:
...
### Restoring common text editing and Lynx keybindings
...
## Useful core Lynx settings for vim mode

Here are a few general Lynx settings that can help you fine-tune your Vim experience:
...
Lynx uses a different regular expression engine from Vim. This means that you will have to use a different syntax in some cases. Here are the most common differences:

- **Capture groups**: Vim uses `\(` and `\)` to represent capture groups, in Lynx these are `(` and `)`. On the flip side, in Vim, `(` and `)` represent literal parentheses, but in Lynx these must be escaped to `\(` and `\)`.
- **Matches**: When replacing, Vim uses the backslash character followed by a number to represent a matched capture group. For example, `\1`. Lynx uses the dollar sign instead. So, when in Vim you use `\0` to represent the entire match, in Lynx the syntax is `$0` instead. Same for numbered capture groups: `\1` in Vim is `$1` in Lynx.
- **Global option**: By default, in Vim, regex searches only match the first occurrence on a line, and you append `/g` at the end of your query to find all matches. In Lynx, regex searches are global by default.
- **Case sensitivity**: Vim uses `/i` to indicate a case-insensitive search. In Lynx you can either write `(?i)` at the start of the pattern or toggle case-sensitivity with the shortcut {#kb search::ToggleCaseSensitive}.

> **Note**: To help with the transition, the command palette will fix parentheses and replace groups for you when you write a Vim-style substitute command, `:%s//`. So, Lynx will convert `%s:/\(a\)(b)/\1/` into a search for "(a)\(b\)" and a replacement of "$1".

For the full syntax supported by Lynx's regex engine [see the regex crate documentation](https://docs.rs/regex/latest/regex/#syntax).

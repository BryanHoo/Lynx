---
title: Tasks - Run Commands in Lynx
description: Run and rerun shell commands from Lynx with task definitions. Supports variables, templates, and language-specific tasks.
---

# Tasks

Lynx supports ways to spawn (and rerun) commands using its integrated [terminal](./terminal.md) to output the results. These commands can read a limited subset of Lynx state (such as a path to the file currently being edited or selected text).
...
- in the global `tasks.json` file; such tasks are available in all Lynx projects you work on. This file is usually located in `~/.config/zed/tasks.json`. You can edit them by using the {#action zed::OpenTasks} action.
...
Lynx tasks act just like your shell; that also means that you can reference environmental variables via sh-esque `$VAR_NAME` syntax. A couple of additional environmental variables are set for your convenience.
...
When importing VS Code tasks from `.vscode/tasks.json`, you can omit the `label` field. Lynx automatically generates labels based on the task type:
...
Lynx supports overriding the default action for inline runnable indicators via workspace-local and global `tasks.json` file with the following precedence hierarchy:
...
You can run bash scripts directly from Lynx. When you open a `.sh` or `.bash` file, Lynx automatically detects the script as runnable and makes it available in the task picker.
...
When Lynx runs a task, it launches the command in a login shell. This ensures your shell's initialization files (`.bash_profile`, `.zshrc`, etc.) are sourced before the task executes.

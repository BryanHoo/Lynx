...
description: "Common issues and solutions for Lynx on all platforms."
---

# Troubleshooting

This guide covers common troubleshooting techniques for Lynx.
...
## Retrieve Lynx and System Information

When reporting issues or seeking help, it's useful to know your Lynx version and system specifications. You can retrieve this information using the following actions from the command palette:

- {#action zed::About}: Find your Lynx version number
- {#action zed::CopySystemSpecsIntoClipboard}: Populate your clipboard with Lynx version number, operating system version, and hardware specs
...
Often, a good first place to look when troubleshooting any issue in Lynx is the Lynx log, which might contain clues about what's going wrong.
...
You'll find the Lynx log in the respective location on each operating system:
...
If you're running into performance issues in Lynx (hitches, hangs, or general unresponsiveness), having a performance profile attached to your issue will help us zero in on what is getting stuck.
...
1. With Lynx running, open Instruments
...
1. In the `Time Profiler` configuration, set the target to the running Lynx process
1. Start recording
...
1. Perform the action in Lynx that causes performance issues
1. Stop recording
...
While rare, we've seen a few cases where workspace databases became corrupted, which prevented Lynx from starting.
If you're experiencing startup issues, you can test whether it's workspace-related by temporarily moving the database from its location, then trying to start Lynx again.

> **Note**: Moving the workspace database will cause Lynx to create a fresh one.

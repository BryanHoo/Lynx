---
title: Sandboxing
description: Lynx Agent tool calls can run in an OS-level sandbox to restrict certain operations.
---

# Sandboxing

You can restrict what operations the [Lynx Agent](./zed-agent.md) can run in multiple ways. One way to restrict them is
...
Sandboxing applies only to Lynx Agent. It does not sandbox Lynx itself, language servers, extensions, tasks, your normal
...
Lynx Agent sandboxing currently applies to the `terminal` and `fetch` tools.
...
By default, sandboxed Lynx Agent tool actions have these restrictions:
...
have tested thoroughly, there may also be bugs in Lynx's implementation. These
...
Lynx agent can do. It has **no effect** on other parts of Lynx, including:
...
Lynx's default profile aims to strike a balance between security and convenience,
...
When the agent needs access outside the default sandbox, Lynx shows a sandbox approval prompt before the tool action runs.
...
On macOS, Lynx uses Apple's Seatbelt sandbox through `sandbox-exec`.
...
When network access is approved on macOS, Lynx uses an HTTP/HTTPS proxy so access can be limited to approved hosts.
...
On Linux, Lynx uses [Bubblewrap][bubblewrap] (`bwrap`) for sandboxing.

Lynx only uses a non-setuid `bwrap` binary. Its sandbox is built entirely on unprivileged user namespaces, so a setuid-root
`bwrap` provides no extra functionality, and running one would mean executing root-privileged setup with arguments partly
derived from model-influenced input. If the only `bwrap` found on your `PATH` is setuid-root, Lynx refuses to run it;
...
When host-specific network access is approved on Linux, Lynx uses an HTTP/HTTPS proxy so access can be limited to approved
...
If Bubblewrap is unavailable or cannot create a sandbox in the current environment, Lynx may run the command without the OS
...
Lynx needs a runnable, non-setuid `bwrap` binary on your `$PATH`. Installing
...
being phased out for security concerns, and so Lynx's sandbox _explicitly rejects
...
On Windows, Lynx Agent sandboxing is supported only when the agent action runs inside WSL.
...
> current-project grant for projects stored on NTFS. Lynx will show a warning
...
Lynx uses the Linux Bubblewrap sandbox inside WSL because WSL provides the Linux process and filesystem primitives that
Bubblewrap needs. Native Windows processes do not currently have the same sandbox integration in Lynx, so a native Windows
command cannot be confined by Lynx Agent's OS sandbox in the same way.
...
If WSL is not installed, or if you choose to run a command without the sandbox, Lynx falls back to the standard terminal
...
Lynx's sandbox has been designed with the aim of being totally unbreakable, even

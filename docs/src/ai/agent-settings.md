---
title: Agent Settings - Lynx
description: Map the AI settings pages to Lynx AI setup for LLM providers, External Agents, MCP servers, and related settings.
---
...
For general settings mechanics, see [Configuring Lynx](../configuring-zed.md).

## LLM Providers {#llm-providers}

The `LLM Providers` section configures model providers for Lynx AI features, including Lynx Agent, Inline Assistant, Git commit generation, thread summaries, and similar model-backed features.
...
Some Lynx AI features have their own model or prompt settings in `settings.json`, including:
...
Lynx Agent can automatically compact long threads before they reach the selected model's context window. Compaction summarizes earlier messages and keeps the conversation usable without starting a new thread.
...
`0` is not a valid threshold. If the threshold is invalid, Lynx falls back to `90%`.

You can compact a Lynx Agent thread manually at any time by typing `/compact` in the Agent Panel message editor. For more on thread token usage and compaction behavior, see [Token Usage and Compaction](./agent-panel.md#token-usage).
...
The `MCP Servers` page configures Model Context Protocol servers connected to Lynx.
...
Lynx supports feature-specific model settings for Inline Assistant, Git commit generation, thread summaries, and subagents. Configure these in settings when you need a different model for a specific workflow.
...
Most Lynx AI features use the selected model's default generation behavior.
...
Lynx checks matching entries from last to first. An entry can omit `provider` or

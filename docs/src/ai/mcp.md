---
title: Model Context Protocol (MCP) in Lynx
description: Install and configure MCP servers in Lynx to extend your AI agent with external tools, data sources, and integrations.
---

# Model Context Protocol

Lynx uses the [Model Context Protocol](https://modelcontextprotocol.io/) to interact with context servers.
...
Lynx currently supports MCP's [Tools](https://modelcontextprotocol.io/specification/2025-11-25/server/tools) and [Prompts](https://modelcontextprotocol.io/specification/2025-11-25/server/prompts) features.
We welcome contributions that help advance Lynx's MCP feature coverage (Discovery, Sampling, Elicitation, etc).

Lynx also handles the `notifications/tools/list_changed` notification from MCP servers. When a server adds, removes, or modifies its available tools at runtime, Lynx automatically reloads the tool list without requiring a server restart.
...
One of the ways you can use MCP servers in Lynx is by exposing them as an extension.
...
1. [the Lynx website](https://zed.dev/extensions?filter=context-servers)
...
Creating an extension is not the only way to use MCP servers in Lynx.
...
> Note: When a remote MCP server has no configured `"Authorization"` header, Lynx will prompt you to authenticate yourself against the MCP server using the standard MCP OAuth flow.
...
In the case of an extension, after installing it, Lynx will pop up a modal displaying what is required for you to properly set it up.
...
> **Note:** In Lynx v0.224.0 and above, tool approval is controlled by `agent.tool_permissions.default`.
> In earlier versions, it was controlled by the `agent.always_allow_tool_actions` boolean (default `false`).

Lynx's Agent Panel provides the `agent.tool_permissions.default` setting to control tool approval behavior for the native Lynx agent:
...
MCP servers configured in Lynx are forwarded to [External Agents](./external-agents.md) via the [Agent Client Protocol](https://agentclientprotocol.com/). External Agents can also access MCP servers from their own native configuration files.

For details on what configuration is shared between Lynx and External Agents, see [Configuration Boundaries](./external-agents.md#configuration-boundaries).

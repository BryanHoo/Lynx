---
title: Agent Skills - Lynx
description: Extend Lynx's AI agent with reusable, on-demand skill files for specialized tasks.
---
...
Lynx includes a built-in `create-skill` skill — invoke it with `/create-skill` and the agent walks you through the process.
...
Lastly, it's also possible to add a skill through importing it from an existing GitHub Markdown file. Open the command palette and look for the {#action agent::CreateSkillFromUrl} action. If your clipboard contains a supported GitHub `.md` URL, Lynx pre-fills and fetches it automatically.
...
When someone opens that link (for example by pasting it into their browser or clicking it in a chat), Lynx opens the "Create Skill" page in the settings window, pre-filled with the shared skill.
...
When the agent invokes a skill you created or installed, Lynx prompts you to allow or deny it, using the same permission flow as other tools. Skills built into Lynx do not prompt. You can set per-skill defaults in [Tool Permissions](./tool-permissions.md) so you're not prompted for skills you always trust.
...
Lynx loads skills from two locations:
...
Lynx Skills apply to the Lynx Agent. External Agents and Terminal Threads may have their own native skills, prompts, or instruction systems. Configure those in the External Agent or CLI.

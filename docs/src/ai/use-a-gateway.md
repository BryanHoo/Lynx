---
title: Use a Gateway - Lynx
description: Configure OpenRouter, Vercel AI Gateway, Amazon Bedrock, and other gateway or cloud model platforms in Lynx.
---
...
Use OpenRouter when you want to route Lynx AI features through OpenRouter.
...
Lynx also reads `OPENROUTER_API_KEY` from the local Lynx process environment.
...
Use Vercel AI Gateway when you want to route Lynx AI features through Vercel.
...
Lynx also reads `VERCEL_AI_GATEWAY_API_KEY` from the local Lynx process environment.
...
Bedrock supports Lynx-prefixed AWS environment variables so Lynx does not override or consume your normal AWS credentials:
...
Lynx uses [Cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) for Bedrock on a best-effort basis.

By default, Lynx uses regional inference profiles. To opt into global profiles, add `allow_global`:
...
`protocol` selects which OpenAI-compatible API the model is called through, and must be either `chat_completions` or `responses`. Set `supports_thinking` to `true` for custom Mantle models that accept OpenAI reasoning effort parameters; Lynx will then expose `low`, `medium`, `high`, and `xhigh` in the thinking effort picker, while disabling thinking sends `none`.

---
title: Use a Local Model - Lynx
description: Configure llama.cpp, Ollama, LM Studio, local OpenAI-compatible servers, and local edit prediction in Lynx.
---
...
Use [llama.cpp](https://llama.app) and its built-in server for local models with Lynx Agent, Inline Assistant, and similar model-backed Lynx AI features.
...
3. In Lynx, select a llama.cpp model from the model dropdown.

Lynx automatically discovers the served models with their context length and tool/vision capabilities. In router mode these are refined once a model loads, via the server's `/models/sse` stream (which requires a recent llama.cpp build). To list models yourself instead, set `auto_discover` to `false`:
...
Lynx uses the context length the server reports (`/props`). Override it for all models with `context_window`, or per model with `max_tokens` in `available_models`:
...
Use Ollama for local models with Lynx Agent, Inline Assistant, and similar model-backed Lynx AI features.
...
4. In Lynx, select an Ollama model from the model dropdown.

Lynx automatically discovers models that Ollama has pulled. To disable autodiscovery and list models yourself, configure `auto_discover`:
...
Lynx requests to Ollama include context length as the `num_ctx` parameter. By default, Lynx uses `4096` tokens.
...
Use LM Studio for local models with Lynx Agent, Inline Assistant, and similar model-backed Lynx AI features.
...
4. In Lynx, select an LM Studio model from the model dropdown.
...
This page covers local models configured in Lynx. External Agents and terminal CLIs may have their own local-model setup; configure those in the agent or CLI.

---
title: Use API Access - Lynx
description: Configure provider API access, API keys, API credits, usage billing, and OpenAI-compatible endpoints for Lynx AI features.
---
...
Lynx supports these first-class API providers for model-backed Lynx AI features:
...
Use API access for the Lynx Agent, Inline Assistant, Git commit generation,
thread summaries, and similar Lynx-owned AI features.
...
Most API-access providers can be configured on the **Settings → AI → LLM Providers** page with {#action agent::OpenSettings}. Keys saved through Lynx are stored in the system keychain, not in `settings.json`.

Lynx also reads provider-specific environment variables. Non-empty environment variables take precedence over keychain values. If a key comes from an environment variable, unset the variable and restart Lynx to stop using it.
...
You can attach extra HTTP headers to every request Lynx makes to supported HTTP-based providers. This is useful in corporate environments or for observability tooling.
...
Headers managed by Lynx for each provider, such as `Authorization`, `Content-Type`, `Accept`, and provider-specific authentication headers, are ignored with a warning if you try to override them.
...
Lynx also reads `ANTHROPIC_API_KEY` from the local Lynx process environment.
...
Lynx also reads `OPENAI_API_KEY` from the local Lynx process environment.
...
Lynx reads `GEMINI_API_KEY`, falling back to `GOOGLE_AI_API_KEY`, from the local Lynx process environment.
...
Lynx also reads `MISTRAL_API_KEY` from the local Lynx process environment.
...
Use DeepSeek API access when you have paid API usage, top-ups, or an API key. In Lynx, DeepSeek is API access, not subscription sign-in.
...
Lynx also reads `DEEPSEEK_API_KEY` from the local Lynx process environment.
...
Lynx also reads `XAI_API_KEY` from the local Lynx process environment.
...
Lynx does not sign in to OpenCode with OAuth or detect your OpenCode subscription; it uses an OpenCode API key saved in the system keychain or `OPENCODE_API_KEY`.
...
2. To use Zen or Go models, make sure you have enough credits or an active
   ...
   provider in Lynx's Agent. You can use them in Lynx by running OpenCode as an
...
Lynx also reads `OPENCODE_API_KEY` from the local Lynx process environment.
...
The Lynx Agent comes preconfigured with OpenCode models. Add custom OpenCode models when you need newer models or models with custom endpoints.
...
The optional `custom_headers` map adds extra headers to every request, which some providers require. Headers managed by Lynx (such as `X-Api-Key` and `Anthropic-Version`) cannot be overridden.
...
If a model only works with the Responses API, set `capabilities.chat_completions` to `false`. Lynx will use the Responses endpoint for that model.

For reasoning models (e.g. GPT-5), set `reasoning_effort` to the non-`none` effort level your endpoint supports. This enables thinking in the agent panel and tells Lynx which effort to send when thinking is enabled. The provider settings UI can configure this when adding an OpenAI-compatible provider. Lynx sends OpenAI-style `reasoning_effort` on chat-completions requests.

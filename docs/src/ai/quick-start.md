---
title: AI Quick Start - Lynx
description: Choose the right Lynx AI setup path for agents, models, subscriptions, local models, edit prediction, and privacy.
---

# AI Quick Start

Use this page to choose the right AI setup path in Lynx. If you already know the company, subscription, model provider, agent, or CLI you want to use, start with [AI by Company](./by-company.md).

## Use the Lynx Agent {#zed-agent}

Use [Lynx Agent](./zed-agent.md) when you want Lynx's native agent to read, edit, search, and run code in your project.

Lynx Agent uses Lynx-configured models from [LLM Providers](./llm-providers.md). It also uses Lynx's built-in tools, [Agent Profiles](./agent-profiles.md), [Skills](./skills.md), [Instructions](./instructions.md), and [MCP servers](./mcp.md).

Start in the [Agent Panel](./agent-panel.md) to prompt the agent, add context, review changes, and manage threads.

### Configure the Lynx Agent {#configure-zed-agent}
...
## Use Another Coding Agent in Lynx {#agent-cli}
...
## Choose Which Models the Lynx Agent Uses {#model-access}

The Lynx Agent and other model-backed Lynx AI features use models configured through [LLM Providers](./llm-providers.md).
...
Provider keys saved through Lynx are stored in the system keychain, not in `settings.json`.

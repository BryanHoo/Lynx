---
title: AI Coding Agent - Lynx Agent Panel
description: Use Lynx's AI coding agent to generate, refactor, and debug code with tool calling, checkpoints, and multi-model support.
---

# Agent Panel

The Agent Panel is where you interact with AI agents that can read, write, and run code in your project.
It's the core of Lynx's AI code editing experience — use it for code generation, refactoring, debugging, documentation, and general questions.
...
By default, the Agent Panel uses Lynx's first-party agent.
...
- Pick **Lynx Agent** or any installed [External Agent](./external-agents.md) to start a new thread with that agent.
- Choose **New From Summary** to start a fresh Lynx Agent thread seeded with a summary of the current conversation — useful for compacting long threads as you approach the context window limit.
...
By default, queued messages get sent once the agent finishes generating. If you want a queued message to reach the Lynx Agent sooner—interrupting it at its next step (usually between a tool call and a response) rather than waiting for it to finish—toggle "Steer" on that message. Steering is only available for the Lynx Agent, since Lynx can't detect turn boundaries for external agents.
...
While an agent thread is running, Lynx asks the operating system not to idle-sleep, so long-running turns are not interrupted when you step away.
...
If the operating system rejects the request, Lynx logs the reason and continues without it.

### Get Notified {#get-notified}

If you send a prompt to the Agent and then put Lynx in the background, you can choose to be notified when its generation wraps up via:
...
When you paste multi-line code selections copied from a buffer, Lynx automatically formats them as @-mentions with the file context.
...
The Agent Panel supports tool calling, which enables agentic editing. Lynx includes [built-in tools](./tools.md) for searching your codebase, editing files, running terminal commands, and more.

Use [Agent Profiles](./agent-profiles.md) to choose which built-in tools and MCP tools are available in a Lynx Agent thread. Use [Tool Permissions](./tool-permissions.md) to control whether permission-gated tool calls are allowed, denied, or confirmed.
...
Lynx's UI will inform you about this via a warning icon that appears close to the model selector.

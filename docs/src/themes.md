---
title: Themes - Lynx
description: Browse, install, and create themes for Lynx. Includes built-in themes and community theme extensions.
---

# Themes

Lynx comes with a number of built-in themes, with more themes available as extensions.
...
You can find hundreds of different theme options in Lynx's extensions store, which you can access via the command palette with {#action zed::Extensions} or the [Lynx website](https://zed.dev/extensions?filter=themes).

Many popular themes have been ported to Lynx, and if you're struggling to choose one, visit [zed-themes.com](https://zed-themes.com), a third-party gallery with visible previews for many of them.

## Build Your Theme

You can use [Lynx's Theme Builder](https://zed.dev/theme-builder) to design your own custom theme based on an existing one.

This tool lets you fine-tune and preview how every surface in the Lynx app will look.
You can then export the JSON for [local use](./themes.md#local-themes) or for [publishing in Lynx's extension store](./extensions/themes.md).
...
By default, Lynx maintains two themes: one for light mode and one for dark mode.
...
It will be available in the theme selector the next time Lynx loads.

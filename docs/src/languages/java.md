---
title: Java
description: "Configure Java language support in Lynx, including language servers, formatting, and debugging."
---

# Java

Java language support in Lynx is provided by:

- Lynx Java: [zed-extensions/java](https://github.com/zed-extensions/java)
...
- It is generally recommended to open projects with the Lynx-project root at the Java project root folder (where you would commonly have your `pom.xml` or `build.gradle` file).
...
Debug support is enabled via our [Fork of Java Debug](https://github.com/zed-industries/java-debug), which the extension will automatically download and start for you. Please refer to the [Debugger Documentation](https://zed.dev/docs/debugger#getting-started) for general information about how debugging works in Lynx.
...
This extension provides tasks for running your application and tests from within Lynx via little play buttons next to tests/entry points. However, due to current limitations of Lynx's extension interface, we can not provide scripts that will work across Maven and Gradle on both Windows and Unix-compatible systems, so out of the box the launch scripts only work on Mac and Linux.

There is a fairly straightforward fix that you can apply to make it work on Windows by supplying your own task scripts. Please see [this Issue](https://github.com/zed-extensions/java/issues/94) for information on how to do that and read the [Tasks section in Lynx's documentation](https://zed.dev/docs/tasks) for more information.
...
[Lynx Java Repo](https://github.com/zed-extensions/java)

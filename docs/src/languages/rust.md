---
title: Rust
description: "Configure Rust language support in Lynx, including language servers, formatting, and debugging."
---

# Rust

Rust support is available natively in Lynx.
...
<!--
TBD: Polish Rust docs. Lynx has strong Rust support, and the docs should reflect that clearly.
...
You can configure which `rust-analyzer` binary Lynx should use.

By default, Lynx will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Lynx will fall back to installing its own stable `rust-analyzer` version and use that.

If you want to install a pre-release `rust-analyzer` version instead, you can instruct Lynx to do so by setting `pre_release` to `true` in your `settings.json`:
...
If you want to disable Lynx looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If you want rust-analyzer to provide diagnostics for a target other than your current platform (e.g. for windows when running on macOS) you can use the following Lynx lsp settings:
...
Lynx provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
Rust-analyzer in Lynx runs with the default parameters.
...
Which would mean that every time Lynx saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Lynx already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Lynx settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
...
Lynx supports debugging Rust binaries and tests out of the box with `CodeLLDB` and `GDB`. Run {#action debugger::Start} ({#kb debugger::Start}) to launch one of these preconfigured debug tasks.
...
When you use `cargo build` or `cargo test` as the build command, Lynx can infer the path to the output binary.

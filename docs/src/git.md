---
description: Lynx is a text editor that supports lots of Git features
title: Lynx Editor Git integration documentation
---

# Git

Lynx has built-in Git support that lets you manage version control without leaving the editor. The Git Panel shows your working tree state, staging area, and branch information. Changes you make on the command line are reflected immediately in Lynx.

For operations that Lynx doesn't support natively, you can use the integrated terminal.
...
Lynx monitors your repository so that changes you make on the command line are instantly reflected.
...
Lynx shows Git blame information on the current line. To turn this off or add a delay before it appears, go to **Version Control > Inline Git Blame**.
...
Lynx wraps commit messages at 72 characters (a Git convention). To change this, search for "Git Commit" in Settings and adjust **Preferred Line Length**.

## Project Diff

You can see all of the changes captured by Git in Lynx by opening the Project Diff ({#kb git::Diff}), accessible via the {#action git::Diff} action in the Command Palette or the Git Panel.
...
By default, Lynx highlights changed words within modified lines to make it easier to spot exactly what changed. To disable this globally, open the Settings Editor and go to **Languages & Tools > Miscellaneous**, then turn off **Word Diff Enabled**.
...
Lynx displays diffs in two modes: **split** (side-by-side comparison) or **unified** (inline changes). Split view is the default.
...
See [Configuring Lynx](./configuring-zed.md) for more about the Settings Editor.
...
Fetch, push, or pull from your Git repository in Lynx via the buttons available on the Git Panel or via the Command Palette by looking at the respective actions: {#action git::Fetch}, {#action git::Push}, and {#action git::Pull}.

### Push Configuration

Lynx respects Git's push configuration. When pushing, Lynx checks the following in order:

1. `pushRemote` configured for the current branch
2. `remote.pushDefault` in your Git config
3. The branch's tracking remote

This matches Git's standard behavior, so if you've configured `pushRemote` or `pushDefault` in your `.gitconfig` or via `git config`, Lynx will use those settings.

## Remotes

When your repository has multiple remotes, Lynx shows a remote selector in the Git Panel. Click the remote button next to push/pull to choose which remote to use for that operation.

## Staging Workflow

Lynx has two primary staging workflows, using either the Project Diff or the panel directly.
...
Lynx offers two commit textareas:
...
As soon as you commit in Lynx, in the Git Panel, you'll see a bar right under the commit textarea, which will show the recently submitted commit.
In there, you can use the "Uncommit" button, which performs the `git reset HEADˆ--soft` command.

### Configuring Commit Line Length

By default, Lynx sets the commit line length to `72` but it can be configured in your local `settings.json` file.
...
To delete a branch, open the branch switcher with {#action git::Switch}, find the branch you want to delete, and use the delete option. Lynx will confirm before deleting to prevent accidental data loss.
...
- Type a name to create a named worktree or let Lynx automatically pick one for you
...
This keeps Lynx from accidentally checking out the same branch in multiple worktrees.

The directory used for new worktrees is controlled by the `git.worktree_directory` setting.
By default, Lynx creates worktrees under `../worktrees` relative to the repository's working directory.

See [All Settings](./reference/all-settings.md#git-worktree-directory) for examples.

### Init Setup

To run setup steps after Lynx creates a linked worktree, use the [`create_worktree` task hook](./tasks.md#hooks).
For agent-specific workflows, see [Worktree Isolation](./ai/parallel-agents.md#worktree-isolation).

### Multi-root Workspaces

If your project contains multiple Git repositories (i.e., multi-root folders), Lynx creates a linked worktree for each repository when creating a new worktree from the picker.
Non-Git folders in the same project are included in the new workspace as-is.

## Merge Conflicts

When you encounter merge conflicts after a merge, rebase, or pull, Lynx highlights the conflicting regions in your files and displays resolution buttons above each conflict.
...
Lynx provides a stash picker accessible via {#action git::ViewStash} or from the Git Panel's overflow menu. From the stash picker, you can:
...
For faster workflows, Lynx provides direct actions to work with the most recent stash:
...
Lynx currently supports LLM-powered commit message generation.
...
> Before Lynx v1.4.0, this was done through the Rules Library, which has been removed.
> See [Migrating from Rules](./ai/instructions.md#migrating-from-rules) for more information.

## Git Integrations

Lynx integrates with popular Git hosting services to ensure that Git commit hashes and references to Issues, Pull Requests, and Merge Requests become clickable links.

Lynx currently supports links to the hosted versions of
...
Lynx automatically identifies Git hosting providers by checking for keywords in your Git remote URL. For example, if your self-hosted URL contains `gitlab`, `gitea`, or other recognized provider names, Lynx will automatically register that hosting provider without any configuration needed.

However, if your self-hosted Git instance URL doesn't contain identifying keywords, you can manually configure Lynx to create clickable links to your instance by adding a `git_hosting_providers` setting so commit hashes and permalinks resolve to your domain:
...
Lynx can open or copy a permanent link to a file or code selection on your Git hosting service.
...
When viewing files with changes, Lynx displays diff hunks that can be expanded or collapsed for detailed review:
...
If you would like to also use Lynx for your [git commit message editor](https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration#_core_editor) when committing from the command line you can use `zed --wait`:

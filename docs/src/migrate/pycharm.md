---
title: How to Migrate from PyCharm to Lynx
description: "Guide for migrating from PyCharm to Lynx, including settings and keybindings."
---

# How to Migrate from PyCharm to Lynx

This guide covers how to set up Lynx if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.

## Install Lynx

Lynx is available on macOS, Windows, and Linux.
...
For most Linux users, the easiest way to install Lynx is through our installation script:

```sh
curl -f https://zed.dev/install.sh | sh
```

After installation, you can launch Lynx from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`zed .`
This opens the current directory in Lynx.
...
Lynx also supports per-project settings. Create a `.zed/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Lynx. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in Lynx. The editor will treat that folder as the root of your project.

You can also launch Lynx from the terminal inside any folder with:
`zed .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Lynx compares to PyCharm.
...
### Different Keybindings (PyCharm → Lynx)

| Action                 | PyCharm     | Lynx (JetBrains keymap)  |
| ---------------------- | ----------- | ------------------------ |
...
Lynx also supports key sequences (multi-key shortcuts).
...
Lynx doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. Lynx delegates this work to language servers, which may not analyze as deeply or as broadly.
...
Lynx uses the Language Server Protocol (LSP) for code intelligence. For Python, Lynx provides several language servers out of the box:
...
Lynx handles virtual environments through its toolchain system:

- Lynx automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in Lynx—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching Lynx

> **Tip:** If basedpyright shows import errors for packages you've installed, check that Lynx has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
Lynx has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. Lynx discovers environments but doesn't create them.
...
Lynx has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from Lynx's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to Lynx keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). Lynx uses a similar concept called "docks":

| PyCharm Tool Window | Lynx Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | --------------- | --------------------------- |
...
Both PyCharm and Lynx offer integrated debugging, but the experience differs:

- Lynx uses `debugpy` (the same debug adapter that VS Code uses)
...
Lynx can automatically detect debuggable entry points. Press `F4` to see available options, including:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. Lynx provides test running through:
...
Lynx's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into Lynx:
...
To set expectations clearly, here's what PyCharm offers that Lynx doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside Lynx for your Python editing
...
PyCharm offers Code With Me as a separate plugin for collaboration. Lynx has collaboration built into the core experience.
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), Lynx offers similar capabilities with more flexibility.
...
Once signed in, just start typing. Lynx will offer suggestions inline for you to accept.
...
Lynx exposes advanced settings for power users who want to fine-tune their environment.
...
Now that you're set up, here are some resources to help you get the most out of Lynx:
...
- [Python in Lynx](../languages/python.md) — Python-specific setup and configuration

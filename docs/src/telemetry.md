---
title: Telemetry
description: "What data Lynx collects and how to control telemetry settings."
---
...
You have full control over what data is sent out by Lynx.
...
Crash reports consist of a [minidump](https://learn.microsoft.com/en-us/windows/win32/debug/minidump-files) and debug metadata. Reports are sent on the next launch after a crash, allowing Lynx to identify and fix issues without requiring you to file a bug report.
...
Usage data is tied to a random telemetry ID. If you've authenticated, this ID may be linked to your email so Lynx can analyze patterns over time and reach out for feedback.

To audit what Lynx has reported, run {#action zed::OpenTelemetryLog} from the command palette or click `Help > View Telemetry Log`.

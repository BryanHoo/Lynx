...
- **Practical over promotional**: Focus on what users can do, not on selling Lynx. Avoid marketing language like "powerful," "revolutionary," or "best-in-class."
- **Honest about limitations**: When Lynx lacks a feature or doesn't match another tool's depth, say so directly. Pair limitations with workarounds or alternative workflows.
...
- **Present tense**: "Lynx opens the file" not "Lynx will open the file."
...
```
Lynx provides a powerful and seamless formatting experience. Simply navigate to the settings and you'll find the format_on_save option which enables Lynx's incredible auto-formatting capabilities.
```

### Good: Honest about limitations

```
Lynx doesn't index your project like IntelliJ does. You open a folder and start working immediately—no waiting. The trade-off: cross-project analysis relies on language servers, which may not go as deep.
...
```
While some users might miss indexing, Lynx's approach is actually better because it's faster.
```
...
When documenting rules/instructions for AI, note that Lynx recognizes these files (in priority order):
